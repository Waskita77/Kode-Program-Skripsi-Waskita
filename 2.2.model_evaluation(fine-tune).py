...
def evaluate_finetune():
    ...
    for name in Config.MODELS:
        ...
        results.append(metrics_only(model, X_tgt_t, y_tgt, name))

    df_out = pd.DataFrame(results).set_index("Model").round(4)
    save_csv(df_out, eval_dir, "evaluation_results_target_test.csv")
